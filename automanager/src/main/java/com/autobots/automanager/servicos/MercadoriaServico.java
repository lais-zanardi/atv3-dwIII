...
public class MercadoriaServico {
    @Autowired private MercadoriaRepositorio repositorio;
    @Autowired
    private MercadoriaAtualizador atualizador;

    public Mercadoria salvar(Mercadoria mercadoria) { return repositorio.save(mercadoria); }
    public List<Mercadoria> obterTodas() { return repositorio.findAll(); }
    public Mercadoria obterPorId(Long id) { return repositorio.findById(id).orElse(null); }
